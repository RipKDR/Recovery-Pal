...
export async function generateWeeklyReport(
  ...
  const averageMood = weekCheckins.length > 0
    ? Math.round(weekCheckins.reduce((sum, c) => sum + c.mood, 0) / weekCheckins.length * 10) / 10
    : 0;
  
  const averageCraving = weekCheckins.length > 0
    ? Math.round(weekCheckins.reduce((sum, c) => sum + c.cravingLevel, 0) / weekCheckins.length * 10) / 10
    : 0;
  ...
  // Meeting stats
  const weekMeetings = meetingLogs.filter(m => 
    new Date(m.date) >= weekStart && new Date(m.date) <= weekEnd
  );
  ...
  const weekCalls = callLogs.filter(c => 
    new Date(c.calledAt) >= weekStart && new Date(c.calledAt) <= weekEnd
  );
  const phoneCalls = weekCalls.length;
  
  // Sponsor contacts
  const sponsorContacts = sponsorContactDate && 
    new Date(sponsorContactDate) >= weekStart ? 1 : 0;
  
  // Step work
  const currentStep = stepProgress.find(p => p.answeredQuestions < p.totalQuestions)?.stepNumber || 12;
  ...
  const weekAchievements = achievements.filter(a => 
    a.unlockedAt && new Date(a.unlockedAt) >= weekStart && new Date(a.unlockedAt) <= weekEnd
  );
  ...
  const keytagEarned = latestKeytag && soberDays - latestKeytag.daysRequired < 7 
    ? latestKeytag.name 
    : null;
  ...
  if (achievementsUnlocked.length > 0) highlights.push(`Unlocked ${achievementsUnlocked.length} achievement(s)`);
...
export function formatReportForDisplay(report: WeeklyReport): string {
  ...
  lines.push(`${report.weekStartDate.toLocaleDateString()} - ${report.weekEndDate.toLocaleDateString()}`);
  lines.push('');
